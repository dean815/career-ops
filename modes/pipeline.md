...
Processes accumulated offer URLs from `data/pipeline.md`. The user adds URLs whenever they want and then runs `/career-ops pipeline` to process them all.
...
2. **For each pending URL**:
   ...
   d. **Run complete auto-pipeline**: A-F Evaluation → Report .md → PDF (if score >= 3.0) → Tracker
